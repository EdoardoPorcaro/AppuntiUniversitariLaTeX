...
% -------[ IMPOSTAZIONE PAGINA, TESTO E CARATTERI ]---------------------------------
	...
	\renewcommand{\appendixname}{Appendice} % Rinomina il "nome" della singola appendice
	\renewcommand{\appendixpagename}{Appendici} % Rinomina il "nome" dell'insieme delle appendici
        \renewcommand{\appendixtocname}{Appendici}} % Rinomina il "nome" dell'insieme delle appendici per il toc
    \usepackage{microtype} % Rende la tipografia più professionale e raffinata
    \usepackage{emptypage} % Rende automaticamente le pagine bianche inserite senza né intestazione né piè di pagina (equivalente a scrivere \thispagestyle{empty} per ciascuna pagina bianca)
...
% -------[ COLLEGAMENTI E SEGNALIBRI ]----------------------------------------------
    ...
    % Imposta le proprietà (i metadati) del documento PDF
        \hypersetup{
            ...
            hidelinks, % Toglie i rettangoli intorno ai collegamenti
	    %plainpages=false,
	    %pdfpagelabels=true,
	    %hypertexnames=true,
            pdftitle = {\NomeCorso},
...
% -------[ APPENDICI E ACRONIMI ]----------------------------------------------
    % Consenti la creazione e la gestione di appendici finali
    \usepackage[title]{appendix}
    % Gestisci gli acronimi con il pacchetto glossaries
    \usepackage{relsize} % Necessario per il funzionamento di \setacronymstyle{long-sm-short}
    \usepackage[acronym,shortcuts]{glossaries} % L'opzione "shortcuts" permette di usare indifferentemente pure i comandi del pacchetto acronyms
    \setacronymstyle{long-sm-short} % Definisco lo stile degli acronimi: prima la forma estesa (alla prima occorrenza) e poi la forma corta/acronimo in maiuscoletto (tra parentesi alla prima occorrenza) 
    \makeglossaries
% -------[ fine ]-------------------------------------------------------------------
...
% -------[ PERSONALIZZAZIONE RIFERIMENTI ]------------------------------------------
    ...
    %   Range di equazioni (eqq. e Eqq.) (tolgo anche le parentesti altrimenti presenti)
        \crefrangeformat{equation}{\sffamily eqq. da #1 a #2}
        \Crefrangeformat{equation}{\sffamily Eqq. da #1 a #2}
    %   Range di figure (figg. e Figg.)
        \crefrangeformat{figure}{\sffamily figg. da #1 a #2}
        \Crefrangeformat{figure}{\sffamily Figg. da #1 a #2}
    %   Range di tabelle (tabb. e Tabb.)
    % TABLE
    \crefrangeformat{table}{\sffamily tabb. da #1 a #2}
    \Crefrangeformat{table}{\sffamily Tabb. da #1 a #2}
    
    % SECTION
    \crefrangeformat{section}{\sffamily capp. da #1 a #2}
    \Crefrangeformat{section}{\sffamily Capp. da #1 a #2}
    
    % SUBSECTION
    \crefrangeformat{subsection}{\sffamily sezz. da #1 a #2}
    \Crefrangeformat{subsection}{\sffamily Sezz. da #1 a #2}
    
    % SUBSUBSECTION
    \crefrangeformat{subsubsection}{\sffamily parr. da #1 a #2}
    \Crefrangeformat{subsubsection}{\sffamily Parr. da #1 a #2}
    
    % PAGE
    \crefrangeformat{page}{pagg. da #1 a #2}
    \Crefrangeformat{page}{Pagg. da #1 a #2}
...
% -------[ DEFINIZIONI MATEMATICHE PERSONALIZZATE ]---------------------------------
    ...
    % Operatori parentesi
    % Usare versione stellata * affinché i delimitatori si dimensionino automaticamente
        \DeclarePairedDelimiter{\abs}{\lvert}{\rvert} % \abs{x} => |x|
        \DeclarePairedDelimiter{\norm}{\lVert}{\rVert} % \norm{x} => ||x||
        \DeclarePairedDelimiter{\set}{\{}{\}} % \set{x} => {x}
    ...
    % Lettere greche preferite (inverto quella di default e quella alternativa)
        % epsilon
            \let\altepsilon\epsilon % ALTernativa (sulla base delle mie preferenze)
            \let\corepsilon\varepsilon % CORretta (sulla base delle mie preferenze)
            \renewcommand{\epsilon}{\corepsilon}
            \renewcommand{\varepsilon}{\altepsilon}
        % phi
            \let\altphi\phi % ALTernativa (sulla base delle mie preferenze)
            \let\corphi\varphi % CORretta (sulla base delle mie preferenze)
